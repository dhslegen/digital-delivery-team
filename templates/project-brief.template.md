...
<!-- §11（v0.9.8 D25）：可选字段。本项目要对接的第三方系统接口契约。
     与"参考资料"区别：参考资料 = 被动浏览的链接；集成依赖 = product-agent 在 /prd 阶段
     主动消费的接口契约（endpoint × 鉴权 × 错误码）。
...
| 项目 | 值 |
|---|---|
| 契约文档 | `项目资料/<目录>/`（**相对路径**，product-agent 会按需读取） |
| 基础 URL | `https://api.example.com/v1` |
| 鉴权方式 | OAuth2 client_credentials（**敏感字段已脱敏**） |
| Endpoint 数 | 14 个核心 + 4 个扩展 |
| 关键能力 | <主要业务能力，1-3 个短语> |
