...
{
  questions: [{
    question: "后端实现 已生成（已实现 endpoint / 集成测试通过率），如何继续？",
...
```bash
node "$DDT_PLUGIN_ROOT/bin/emit-decision.mjs" --phase build-api --action resolved \
...
| 用户选择 | 行为 |
|---------|------|
| 接受并继续 | 走"标记阶段完成"段落（emit-phase end）+ 提示用户运行 `/build-web 或 /verify` |
