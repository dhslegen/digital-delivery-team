...
if ! printf '%s' "$ARGUMENTS" | grep -q -- '--auto'; then
  node "$DDT_PLUGIN_ROOT/bin/emit-decision.mjs" --phase test --action point \
...
{
  questions: [{
    question: "测试报告 已生成（覆盖率 / 通过率 / 失败用例），如何继续？",
    header: "Test review",
    multiSelect: false,
    options: [
      { label: "接受并继续 (Recommended)",
         description: "进入 /review 或 /verify",
...
```bash
node "$DDT_PLUGIN_ROOT/bin/emit-decision.mjs" --phase test --action resolved \
...
| 用户选择 | 行为 |
|---------|------|
| 接受并继续 | 走"标记阶段完成"段落（emit-phase end）+ 提示用户运行 `/review 或 /verify` |
