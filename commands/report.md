...
if printf '%s' "$ARGUMENTS" | grep -q -- '--dry-run'; then
  node "$DDT_PLUGIN_ROOT/bin/print-dry-run.mjs" --phase report --inputs "~/.claude/delivery-metrics/events.jsonl,baseline/historical-projects.csv,.ddt/progress.json" --outputs "docs/efficiency-report.md" --next "/ship"
  exit 0
fi
