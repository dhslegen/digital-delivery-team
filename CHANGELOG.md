...
**4 个新 skill（替代 2 个旧 agent + 抽取 DDT 范式）**
...
**M6.5 跨会话接力 skill**
- `skills/relay/SKILL.md` — DDT 13 段式接力范式 + 项目特色注入
...
完整修复 v0.4.x e2e 测试中发现的 P0 数据采集断链 + 体验对齐 + 技术栈灵活性 + 可重入状态机。
...
- Component inventory via `bin/manifest.mjs`; plugin.json 不声明 agents/hooks（T-R07）
...
- Hook runtime 独立为 DDT 命名空间：`DDT_HOOK_PROFILE` / `DDT_DISABLED_HOOKS`，不读取其他插件开关。
